...
func main() {
	...
	for {
		...
		for scanner.Scan() {
			ln := scanner.Text()
			fmt.Println(ln) //I tried writing back to the connection with io.WriteString but it didnt show up in the telnet terminal
			//The point of this was to get caught in the loop
		}
